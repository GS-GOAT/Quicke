...
class ContextTracker {
  ...
  /**
   * a model's response to the context
   */
  ...
  formatContextForModel(threadId, conversationId, currentPrompt, modelType) {
    ...
    // If we have a current prompt, it temporarily to the context
  ...
  formatForGemini(context) {
    ...
    // First system instruction as user message if we have context
    if (context.length > 0) {
      messages.push({
        role: 'user',
